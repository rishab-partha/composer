...
class ModelBasedDataset(Dataset):
    """A dataset that construct batches for in-context learning model based evaluation

    The input format is expected to be a jsonl file with the following fields:
    - prompt: the question
    - context: background information for the prompt
    - continuation: continuation of the prompt

    Args:
        dataset_uri (str): Either a local path, or a remote path beginning with ``s3://``, or another backend
            supported by :meth:`composer.utils.maybe_create_object_store_from_uri`. Dataset must consist of rows of JSON data points with "prompt",
            "context".
        tokenizer (Union[transformers.PreTrainedTokenizer, transformers.PreTrainedTokenizerFast]): The tokenizer used to map between strings and token ids
        batch_size (int): Size of a batch used for eval
        max_seq_len (int): The maximum sequence length supported by the model
    ...
    def prep_examples(self, num_fewshot: int, prompt_string: str, example_delimiter: str, continuation_delimiter: str,
        ...
        Each task consists of a context, prompt, and a continuation as well as an optional prompt and optional list of
        example context/prompt/continuation pairs which precede the test context/prompt/continuation pair.
        ...
        Returns:
            dict: Contains the context, the propmt, the continuation, and the preamble (prompt + fewshot examples)
        """
        max_answer_length = 0
        examples = []
        for sample_idx in tqdm(range(len(self.samples))):
            ...
            if num_fewshot > 0:
                fewshot_idxs = _get_fewshot_sample_idxs(len(self.samples), num_fewshot, sample_idx, fewshot_rng)
                for fewshot_idx in fewshot_idxs:
                    ctxt, prompt, cont = self.samples[fewshot_idx]['context'], self.samples[fewshot_idx]['prompt'], self.samples[fewshot_idx]['answer']
                    ctxt = f'{ctxt}{question_prelimiter}{prompt}'
                    if len(preamble) > 0:
                        ctxt = f'{example_delimiter}{ctxt}'
                    preamble += f'{ctxt}{continuation_delimiter}{cont}'

            ctxt = self.samples[sample_idx]['context']
            prompt = self.samples[sample_idx]['prompt']
            ctxt = f'{ctxt}{question_prelimiter}{prompt}'
            if len(preamble) > 0:
                ctxt = f'{example_delimiter}{ctxt}'
            ...
            encoded_example['context'] = self.tokenizer(ctxt, add_special_tokens=False)
            encoded_example['continuation'] = self.samples[sample_idx]['continuation']
            encoded_example['prompt'] = self.samples[sample_idx]['prompt']
    ...
    def collate_fn(self, data):
        inputs, answers, prompts = [], [], []

        for sample in data:
            preamble, context, continuation, prompt = (sample['preamble'], sample['context'], sample['continuation'], sample['prompt'])
            context_enc = preamble['input_ids'] + context['input_ids']
            inp, _ = _make_padded_input(context_enc, [],
                                        self.max_seq_len - self.max_answer_length,
                                        self.pad_tok_id,
...
def build_icl_dataloader(
    ...
    if isinstance(
            dataset,
        (InContextLearningMultipleChoiceTaskDataset, InContextLearningQATaskDataset, InContextLearningCodeEvalDataset, ModelBasedDataset)):
