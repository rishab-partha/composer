...
from composer.metrics import InContextLearningCodeEvalAccuracy, InContextLearningMetric, InContextLearningRelevanceAccuracy  
...
class HuggingFaceModel(ComposerModel):
    ...
    def update_metric(self, batch: Any, outputs: Any, metric: Metric) -> None:
        if (isinstance(metric, InContextLearningMetric) and batch.get('mode', None) == 'icl_task') or isinstance(
                metric, InContextLearningCodeEvalAccuracy) or isinstance(
                metric, InContextLearningRelevanceAccuracy):
            assert self.labels is not None
